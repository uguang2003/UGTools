...
import top.ug666.ug.tools.util.*;
        ...
        dbFilePathSaveButton.addActionListener(e -> {
            try {
                ...
                if (StringUtils.isNotBlank(dbFilePath)) {
                    FileUtil.copy(dbFilePathBefore + File.separator + "UGTools.db", dbFilePath, false);
                }
                ...
                App.config.save();
                JOptionPane.showMessageDialog(contentPane, "保存成功！\n\n需要重启UGTools生效", "成功", JOptionPane.INFORMATION_MESSAGE);
                JOptionPane.showMessageDialog(contentPane, "UGTools即将关闭！\n\n关闭后需要手动再次打开", "UGTools即将关闭", JOptionPane.INFORMATION_MESSAGE);
                System.exit(0);
            } catch (Exception e1) {
